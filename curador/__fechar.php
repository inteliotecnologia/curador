...
switch ($_SESSION["id_turno_sessao"]) {
	case 1:
...
$result_soma= mysql_query("select sum(peso) as soma from op_limpa_pesagem, rh_turnos
							...
							and   rh_turnos.id_turno_index = '". $_SESSION["id_turno_sessao"] ."'
...
$funcionarios_neste_turno_neste_dia= pega_funcionarios_trabalhando_retroativo(1, $_SESSION["id_turno_sessao"], $data, $data ." ". $rs_dia->entrada);
...
<table cellspacing="0">
    ...
    <tr>
    	<td><?= $data; ?></td>
        <td><?= pega_turno_padrao($_SESSION["id_turno_sessao"]); ?></td>
